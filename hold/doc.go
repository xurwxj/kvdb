/*
Package hold is an indexing and querying layer on top of a badger DB.  The goal is to allow easy, persistent storage
and retrieval of Go types.  badgerDB is an embedded key-value store, and hold serves a similar use case however with
a higher level interface for common uses of Badger.

Go Types

hold deals directly with Go Types.  When inserting data, you pass in your structure directly.  When querying data you
...
hold will automatically create an index for any struct fields tags with "holdIndex"
...
Queries are chained together criteria that applies to a set of fields:
	...
	hold.Where("Name").Eq("John Doe").And("DOB").Lt(time.Now())
