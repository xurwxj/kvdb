...
var nestedTests = []test{
	test{
		name:   "Nested",
		query:  hold.Where("L1.Name").Eq("Joe"),
		result: []int{0},
	},
	test{
		name:   "Embedded",
		query:  hold.Where("Color").Eq("red"),
		result: []int{0, 1},
	},
	test{
		name:   "Embedded Explicit",
		query:  hold.Where("Embed.Color").Eq("red"),
		result: []int{0, 1},
	},
	test{
		name:   "Nested Multiple Levels",
		query:  hold.Where("L2.L3.Name").Eq("Joe"),
		result: []int{0, 3},
	},
	test{
		name:   "Pointer",
		query:  hold.Where("Pointer.Name").Eq("Jill"),
		result: []int{1, 2, 3},
	},
	test{
		name:   "Sort",
		query:  hold.Where("Key").Ge(0).SortBy("L2.L3.Name"),
		result: []int{4, 1, 2, 0, 3},
	},
	test{
		name:   "Sort On Pointer",
		query:  hold.Where("Key").Ge(0).SortBy("Pointer.Name"),
		result: []int{4, 1, 2, 0, 3},
	},
}
