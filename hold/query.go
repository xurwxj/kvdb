...
// Key is shorthand for specifying a query to run again the Key in a hold, simply returns ""
// Where(hold.Key).Eq("testkey")
const Key = ""

// Query is a chained collection of criteria of which an object in the hold needs to match to be returned
...
// Where starts a query for specifying the criteria that an object in the hold needs to match to
// be returned in a Find result
/*
Query API Example

	s.Find(hold.Where("FieldName").Eq(value).And("AnotherField").Lt(AnotherValue).
		Or(hold.Where("FieldName").Eq(anotherValue)
...
// Record is the complete record for a given row in hold
...
func findQuery(tx *badger.Txn, result interface{}, query *Query) error {
	...
	for i := 0; i < tp.NumField(); i++ {
		if strings.Contains(string(tp.Field(i).Tag), HoldKeyTag) ||
			tp.Field(i).Tag.Get(holdPrefixTag) == holdPrefixKeyValue {
