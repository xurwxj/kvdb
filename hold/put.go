...
var ErrKeyExists = errors.New("This Key already exists in hold for this type")
...
// sequence tells hold to insert the key as the next sequence in the bucket
type sequence struct{}

// NextSequence is used to create a sequential key for inserts
// Inserts a uint64 as the key
// store.Insert(hold.NextSequence(), data)
func NextSequence() interface{} {
	return sequence{}
}

// Insert inserts the passed in data into the the hold
//
// If the the key already exists in the hold, then an ErrKeyExists is returned
// If the data struct has a field tagged as `holdKey` and it is the same type
...
//
// To use this with hold.NextSequence() use a type of `uint64` for the key field.
...
// Update updates an existing record in the hold
