...
func TestIssue36(t *testing.T) {
	testWrap(t, func(store *hold.Store, t *testing.T) {
		...
		data1 := []*Tag1{{}, {}, {}}
		for i := range data1 {
			ok(t, store.Insert(hold.NextSequence(), data1[i]))
			equals(t, uint64(i), data1[i].ID)
		}

		data2 := []*Tag2{{}, {}, {}}
		for i := range data2 {
			ok(t, store.Insert(hold.NextSequence(), data2[i]))
			equals(t, uint64(i), data2[i].ID)
		}

		data3 := []*Tag3{{}, {}, {}}
		for i := range data3 {
			ok(t, store.Insert(hold.NextSequence(), data3[i]))
			equals(t, uint64(i), data3[i].ID)
		}

		data4 := []*Tag4{{}, {}, {}}
		for i := range data4 {
			ok(t, store.Insert(hold.NextSequence(), data4[i]))
		...
		for i := range data1 {
			var find1 []*Tag1
			ok(t, store.Find(&find1, hold.Where(hold.Key).Eq(data1[i].ID)))
		...
		for i := range data2 {
			var find2 []*Tag2
			ok(t, store.Find(&find2, hold.Where(hold.Key).Eq(data2[i].ID)))
		...
		for i := range data3 {
			var find3 []*Tag3
			ok(t, store.Find(&find3, hold.Where(hold.Key).Eq(data3[i].ID)))
		...
		for i := range data4 {
			var find4 []*Tag4
			ok(t, store.Find(&find4, hold.Where(hold.Key).Eq(data4[i].ID)))
